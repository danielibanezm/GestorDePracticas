...
public class EditarNecesidad extends JDialog {
	...
	public EditarNecesidad(Necesidad necesidad, boolean esAdmin) {
		setResizable(false);
		setModal(true);
		...
		if (necesidad != null) {
			spinnerDam.setValue(necesidad.getCantidadDam());
			spinnerDaw.setValue(necesidad.getCantidadDaw());
			spinnerAsir.setValue(necesidad.getCantidadAsir());
			spinnerMarke.setValue(necesidad.getCantidadMarketing());
			spinnerFinanzas.setValue(necesidad.getCantidadFinazas());
			if (necesidad.getConvocatoria().equals("ABRIL")) {
		...
		btnAceptar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				c.actualizarNecesidad(rellenaObjeto(), necesidad.getIdNecesidad());
				dispose();
			}
		});
