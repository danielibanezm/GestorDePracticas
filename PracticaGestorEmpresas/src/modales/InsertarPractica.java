...
public class InsertarPractica extends JDialog {
	...
	public InsertarPractica(PracticasVentana ventana, int idCentro) {
		...
		btnContinuar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (idEmpresa != -1 && idAlumno != -1 && formattedTextFieldFechaFinal.getText().charAt(0) != ' '
						&& formattedTextFieldFechaInicial.getText().charAt(0) != ' ') {
					...
					String inicio = "";
					try {
						fin = formatoFecha.format(formatoFecha.parse(formattedTextFieldFechaFinal.getText()));
						inicio = formatoFecha.format(formatoFecha.parse(formattedTextFieldFechaInicial.getText()));
					} catch (ParseException e1) {
						// TODO Bloque catch generado automáticamente
						e1.printStackTrace();
					}

					fechaFinal = java.sql.Date.valueOf(fin);
					fechaInicio = java.sql.Date.valueOf(inicio);

					InsertarAnexos dialog = new InsertarAnexos(ventana, dialogActual, idCentro);
					dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
					dialog.setVisible(true);
					setVisible(false);
				} else {
					JOptionPane.showMessageDialog(null, "Los datos son incorrectos o incompletos", "Error",
